	...
	PF_LayerDef		*output )
{
	...
	PF_ADD_ANGLE("Angle Name", 180, ANGLE_DISK_ID);

	AEFX_CLR_STRUCT(def);

	PF_ADD_BUTTON("Param Name", "Button Name", NULL, NULL, BUTTON_DISK_ID);

	AEFX_CLR_STRUCT(def);

	PF_ADD_CHECKBOXX("Checkbox Name", TRUE, NULL, CHECKBOX_DISK_ID);

	AEFX_CLR_STRUCT(def);

	PF_ADD_COLOR("Colour Name", 255, 0, 0, COLOR_DISK_ID);

	AEFX_CLR_STRUCT(def);

	PF_ADD_FLOAT_SLIDERX("Slider Name", 0, 100, 0, 100, 50, PF_Precision_INTEGER, NULL, NULL, SLIDER_DISK_ID);

	AEFX_CLR_STRUCT(def);

	PF_ADD_LAYER("Layer Name", 1, LAYER_DISK_ID);

	AEFX_CLR_STRUCT(def);

	PF_ADD_POINT("Point Name", 1920, 1080, NULL, POINT_DISK_ID);

	AEFX_CLR_STRUCT(def);

	PF_ADD_POINT_3D("Point 3D Name", 1920, 1080, 500, POINT3D_DISK_ID);

	AEFX_CLR_STRUCT(def);

	PF_ADD_TOPIC("Topic Name", TOPIC_START_DISK_ID);

	PF_ADD_FLOAT_SLIDERX("Topic Slider", 0, 100, 0, 100, 50, PF_Precision_INTEGER, NULL, NULL, TOPIC_SLIDER_DISK_ID);
